...
The intuition I got for building up to the concept of a markov blanket is from this #link("https://kuanjh123.medium.com/markov-blanket-8aaa416495c3")[medium post]. We start with the markov property. This just captures the intuition of a transition that only depends on the current value; if we know the state of a system at present then we have as much information about it as can be known by having access to all previous states.

*Definition 1:* A discrete time process is said to have the markov property if it satisfies the following:
$ Pr(X_(n+1) = x | X_n = x_n) = Pr(X_(n+1) = x | X_1 = x_1, X_2 = x_2, dots, X_n = x_n) $

Instead of just imagining a chain of these, we can imagine a whole network with each node representing a probability of an event and the edges represent conditional dependence. Interesting aside, *how does this relate to automata theory and probabilistic finite state automata?*

We then identify three types of structures found in Bayesian networks:
+ Causal Chain: Parent and child are initially dependent. On certain knowledge of intermediate node they become independent.
+ Common Cause: Children nodes initially dependent. On certain knowledge of the parent they become independent.
+ Common Effect: Parent nodes are initially independent. On certain knowledge of child they become dependent.
